...
각 사업팀(권역)별로 한 페이지(`<team>/index.html`)가 생성된다. 한 페이지 안에 다음 14개 레이어가 **사이드바 토글**로 켜고 끌 수 있는 형태로 들어 있다.
...
### 1.5 인터랙션 도구
- **거리/반경 측정**: 클릭 후 폴리라인 또는 원형 반경. 누적 거리/반경을 카드로 표시.
- **시뮬레이션**: 임의 좌표에 가상 존을 찍어 반경 0.5km 내 수요·경쟁·예상 매출 추정 (`simulate_zone` + LLM 평가)
- **D2D 도착지 분포**: 특정 존 클릭 시 부름 도착지 분포 점도(`d2dDestLayer`)
- **사이드바 리스트**: Gap / 재진입 리스트 클릭 → 지도 자동 이동 + popup
- **인증**: `server.py`로 로컬 호스팅 시 비밀번호 쿠키 기반 인증 (GitHub Pages 빌드는 인증 없음)
...
├── start_ngrok.sh             # ngrok 터널 (외부 임시 공개용)
├── <team_id>/
...
```bash
python3 server.py    # http://localhost:8080  (비밀번호 보호)
```

`server.py`는 단순 정적 서빙 + 비밀번호 쿠키 + 백그라운드 업데이트 API를 가지고 있다.
...
- 사이드바 토글, popup, 시뮬레이션, 측정 모두 사용 가능.
...
4. **시뮬레이션은 LLM 호출 (`evaluate_simulation_llm`)** — 외부 API 키 + 비용 발생. 현재 GitHub Pages 배포본에는 클라이언트 측 시뮬레이션 UI만 있고 평가는 서버 측 호출.
